...
confirmButton.addEventListener('click', function () {
    ...
    const pricePerShare = document.getElementById('pricePerShare').innerText;
    ...
    if (json.length === 0) {
        ...
        for (let index = 0; index < json.length; index++) {
            html += `<div>
                        ...
                        <span>${json[index].quantity * json[index].price}</span>
...
const remove = async (e) => {
    ...
    await fetch('http://localhost:3000/currentHolding/remove', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
        },
